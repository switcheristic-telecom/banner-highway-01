...
function migrateBannersTable(db: Database) {
  ...
  const insertNew = db.prepare(
    `INSERT INTO banners_new (id, branch_id, t, side, angle, asset_id,
  ...
  for (const b of oldBanners) {
    insertNew.run(
      b.id, b.branch_id, b.t, b.side, b.angle,
      assetIdMap.get(b.image_file) ?? null,
      b.distance_factor, b.size_factor, b.elevation_factor, b.emissive_factor
    );
  }
  ...
  console.log(`Migrated ${oldBanners.length} banners, created ${imageFiles.size} assets`);
}
